...
export interface BotAvatarFacePreset {
  label: string;
  color: string;
  face: BotAvatarFace;
  faceX: number;
...
const SHAPE_PATHS: Record<BotAvatarType, string> = {
  ...
  droid:
    'M16 50C16 38.95 24.95 30 36 30H64C75.05 30 84 38.95 84 50V70C84 81.05 75.05 90 64 90H36C24.95 90 16 81.05 16 70ZM4 62A7 7 0 1 1 18 62A7 7 0 1 1 4 62ZM82 62A7 7 0 1 1 96 62A7 7 0 1 1 82 62Z',
  mech:
    'M10 48C10 38.06 18.06 30 28 30H72C81.94 30 90 38.06 90 48V70C90 79.94 81.94 88 72 88H28C18.06 88 10 79.94 10 70ZM3 54C3 51.79 4.79 50 7 50H11V72H7C4.79 72 3 70.21 3 68ZM89 50H93C95.21 50 97 51.79 97 54V68C97 70.21 95.21 72 93 72H89Z',
  alien: 'M50 10C70 10 83 27 83 46C83 65 64 92 50 92C36 92 17 65 17 46C17 27 30 10 50 10Z',
  hexagon:
    'M91.4 45.5A9 9 0 0 1 91.4 54.5L74.6 83.61A9 9 0 0 1 66.8 88.11L33.2 88.11A9 9 0 0 1 25.4 83.61L8.6 54.5A9 9 0 0 1 8.6 45.5L25.4 16.39A9 9 0 0 1 33.2 11.89L66.8 11.89A9 9 0 0 1 74.6 16.39L91.4 45.5Z',
  ...
  cloud:
    'M19 44A25 25 0 1 1 69 44A25 25 0 1 1 19 44ZM47 50A21 21 0 1 1 89 50A21 21 0 1 1 47 50ZM7 68A17 17 0 1 1 41 68A17 17 0 1 1 7 68ZM32 73A18 18 0 1 1 68 73A18 18 0 1 1 32 73ZM60 70A16 16 0 1 1 92 70A16 16 0 1 1 60 70Z',
...
/**
 * Palette + face placement per type. Colors follow the README's Types
 * section (sky, pink, magenta, …) using the library's shipped hexes where
 * they match; flower/cat/hexagon/pebble/puddle use the README hue names
 * over the source values that still read correctly on a dark canvas.
 */
export const botAvatarPresets: Record<BotAvatarType, BotAvatarFacePreset> = {
  clover: { label: 'Clover', color: '#35B8FF', face: 'eyes', faceX: 50, faceY: 50, faceScale: 1 },
  flower: { label: 'Flower', color: '#FF6BC1', face: 'eyes', faceX: 50, faceY: 51, faceScale: 0.95 },
  ...
  hexagon: { label: 'Hexagon', color: '#FF9F1C', face: 'eyes', faceX: 50, faceY: 50, faceScale: 0.95 },
  cat: { label: 'Cat', color: '#FFD166', face: 'eyes', faceX: 50, faceY: 58, faceScale: 1 },
  ...
  pebble: { label: 'Pebble', color: '#FFB088', face: 'eyes', faceX: 50, faceY: 50, faceScale: 0.95 },
  puddle: { label: 'Puddle', color: '#FF7A6B', face: 'eyes', faceX: 50, faceY: 50, faceScale: 0.95 },
};

/** Body colour by type — the palette on its own. */
export const botAvatarPalette: Record<BotAvatarType, string> = Object.fromEntries(
  Object.entries(botAvatarPresets).map(([t, p]) => [t, p.color])
) as Record<BotAvatarType, string>;

export const BOT_AVATAR_TYPES = Object.keys(botAvatarPresets) as BotAvatarType[];
export const BOT_AVATAR_FACES: BotAvatarFace[] = ['eyes', 'mouth'];
export const BOT_AVATAR_STATES: BotAvatarState[] = ['default', 'working', 'sleeping'];
...
/** Path2D builders — one entry per body type (cached). */
export const botAvatarShapes: Record<BotAvatarType, () => Path2D> = Object.fromEntries(
  BOT_AVATAR_TYPES.map((t) => [t, () => cachedPath(SHAPE_PATHS[t])])
) as Record<BotAvatarType, () => Path2D>;

export function getBotAvatarShape(type: BotAvatarType): Path2D {
  return cachedPath(SHAPE_PATHS[type] ?? SHAPE_PATHS.clover);
}

export function getBotAvatarParts(type: BotAvatarType): Path2D | undefined {
  const d = SHAPE_PARTS[type];
  return d ? cachedPath(d) : undefined;
}

export function getBotAvatarPreset(type: BotAvatarType): BotAvatarFacePreset {
  return botAvatarPresets[type] ?? botAvatarPresets.clover;
}

/** Raw SVG path data (for custom exporters / tests). */
export const SHAPE_PATH_DATA = SHAPE_PATHS;
export const SHAPE_PARTS_DATA = SHAPE_PARTS;
